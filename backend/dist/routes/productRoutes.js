...
router.get('/admin', authenticate, authorize([Role.ADMIN]), async (req, res) => {
    try {
...
router.patch('/admin/:id/status', authenticate, authorize([Role.ADMIN]), async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/post-product', authenticate, async (req, res) => {
    try {
