    ...
    try {
        ...
        if (status === 'CONFIRMED') {
            if (!estimatedCompletionDate)
                return res.status(400).json({ error: "Estimated completion date required" });
            ...
            // New Handmade Logic
            if (!shippingMethod)
                return res.status(400).json({ error: "Shipping method required (TRACKED or UNTRACKED)" });
            if (!proofPhotos)
                return res.status(400).json({ error: "Proof photos required (Item + Package)" });
            // Validate photos
            let photosArray = [];
            try {
            ...
            if (shippingMethod === 'TRACKED') {
                if (!trackingNumber)
                    return res.status(400).json({ error: "Tracking number required for tracked shipping" });
            ...
            timelineTitle = 'In Production';
        else if (status === 'READY_TO_SHIP')
            timelineTitle = 'Ready to Ship';
...
const extendOrderGuarantee = async (req, res, next) => {
    try {
        const user = req.user;
        const { id } = req.params;
        if (!user)
        ...
        if (order.customerId !== user.id)
            return res.status(403).json({ error: "Forbidden" });
        if (!order.autoConfirmAt)
            return res.status(400).json({ error: "Order suggests no guarantee period active" });
