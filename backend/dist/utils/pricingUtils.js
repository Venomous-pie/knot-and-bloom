export const CalculateDiscount = (priceInput) => {
    ...
    if (priceInput.discountedPercentage === null ||
    ...
    if (!isFinite(discountPercentage) ||
        discountPercentage <= 0 ||
 ...
 */
export function findLowestPrice(product, variants) {
 ...
 */
export function calculateVariantStoredDiscountedPrice(variantPrice, variantDiscount, productDiscount) {
    if (variantPrice === null || variantPrice === undefined)
        return null;
    const discount = variantDiscount ?? productDiscount;
    if (!discount || discount <= 0)
        return null;
    return Number((Number(variantPrice) * (1 - discount / 100)).toFixed(2));
}
export default {
    calculateFinalPrice,
    findLowestPrice,
    calculateVariantStoredDiscountedPrice,
    CalculateDiscount
};
