...
import { getProductsQuerySchema, productSchema } from "../validators/productValidator.js";
export const postProduct = async (input, user) => {
    ...
    if (user) {
        ...
        if (user.role === Role.SELLER || (user.role !== Role.ADMIN && sellerId)) {
            ...
            if (!sellerId) {
                if (!user.email)
                    throw new Error("Admin email required for seller profile creation");
    ...
    if (sellerId) {
        ...
        if (!seller || seller.status === SellerStatus.BANNED || seller.status === SellerStatus.SUSPENDED) {
            if (!seller)
                throw new ErrorHandler.ValidationError([{ message: "Invalid seller ID", path: ['sellerId'] }]);
                    ...
                    for (const variant of variantsData) {
                        ...
                        await tx.productVariant.create({
                            data: {
                                ...
                                discountedPrice: Pricing.calculateVariantStoredDiscountedPrice(Number(variant.price), Number(variant.discountPercentage), Number(parsedInput.discountPercentage)),
...
export const updateProductStatus = async (productId, status, rejectionReason) => {
...
export const updateProduct = async (productId, input, user) => {
    const parsedId = parseInt(productId);
    if (isNaN(parsedId)) {
        throw new ErrorHandler.ValidationError([{ message: "Invalid product ID", path: ['productId'] }]);
    }
