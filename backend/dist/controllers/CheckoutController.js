    ...
    try {
        ...
        const cart = await prisma.cart.findUnique({
            ...
            include: {
                items: {
                    where: {
                        uid: { in: selectedItemIds.map((id) => Number(id)) },
                    },
                    include: {
                        product: {
                            include: { seller: true }
        ...
        const sessionResult = await prisma.$transaction(async (tx) => {
            ...
            for (const item of cart.items) {
                if (item.productVariant) {
                    ...
                    if (availableStock < item.quantity) {
                        stockIssues.push(`"${item.product.name}" (${item.productVariant.name}): Only ${availableStock} available, requested ${item.quantity}`);
                    }
                }
            }
            ...
            let totalAmount = 0;
            const lockedPrices = cart.items.map(item => {
                const product = {
        ...
        const sellerIds = [...new Set(lockedPrices.map(item => item.sellerId).filter((id) => id !== null))];
