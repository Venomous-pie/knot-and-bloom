...
export const sellerController = {
    ...
    async listActiveSellers(req, res) {
        try {
            const limit = Math.min(parseInt(req.query.limit) || 20, 100);
            const offset = parseInt(req.query.offset) || 0;
            const where = {
    ...
    async getOwnProducts(req, res) {
        try {
            ...
            allProducts.forEach(p => {
                ...
                const hasMultipleTags = p.tags && p.tags.length >= 3;
                if (hasLongName)
                    score += 10;
                if (hasTags)
                    score += 8;
                ...
                const hasMaterials = p.materials && p.materials.trim().length > 0;
                const hasCareInstructions = !!(p.careInstructions && p.careInstructions.trim().length > 0);
                if (hasGoodDesc)
                    score += 10;
                if (hasMaterials)
                    score += 5;
                ...
                const hasProcessingTime = !!(p.processingTime && p.processingTime.trim().length > 0);
    ...
    async markWelcomeSeen(req, res) {
        try {
            ...
            if (!sellerId)
                return res.status(401).json({ error: "Unauthorized - Seller profile not found" });
            ...
            const payload = {
                ...
                ...(updatedCustomer.sellerProfile?.status && { sellerStatus: updatedCustomer.sellerProfile.status }),
            };
        ...
        try {
            ...
            totalOrdersDistribution.forEach(group => {
                if (group.status === 'PENDING')
                    orderCounts.PENDING += group._count.uid;
                else if (['IN_PRODUCTION', 'READY_TO_SHIP'].includes(group.status))
                    orderCounts.PROCESSING += group._count.uid;
                else if (group.status === 'COMPLETED')
                    orderCounts.COMPLETED += group._count.uid;
                else if (['CANCELLED', 'REJECTED'].includes(group.status))
                    orderCounts.CANCELLED += group._count.uid;
            });
            ...
            for (let i = 0; i < 7; i++) {
                ...
                const dayOrders = recentOrders.filter(o => o.uploaded.toISOString().split('T')[0] === dateString);
                salesGraph.push({
                    date: dateString,
